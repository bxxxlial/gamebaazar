<?php include("template/cabecera.php"); 
  ...
  $sentenciaSQL = $conexion->prepare("SELECT c.*,j.NombreJuegos,j.PrecioJuegos,j.ImagenJuegos FROM carrito AS c JOIN juegos AS j ON j.IdJuegos = c.juegos_id WHERE c.usuario_id=:id AND c.estado = '0'");
  $sentenciaSQL->bindParam(':id', $_SESSION['idCliente']);
  $sentenciaSQL->execute();
  $carrito = $sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);

  $sentenciaSQL = $conexion->prepare("SELECT c1.*,a.NombreAccesorios,a.PrecioAccesorios,a.ImagenAccesorios FROM carrito AS c1 JOIN accesorios AS a ON a.IdAccesorios = c1.juegos_id WHERE c1.usuario_id=:id AND c1.estado = '0'");
  $sentenciaSQL->bindParam(':id', $_SESSION['idCliente']);
  $sentenciaSQL->execute();
  $carrito1 = $sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);

  $sentenciaSQL = $conexion->prepare("SELECT c2.*,d.NombreDlcs,d.PrecioDlcs,d.ImagenDlcs FROM carrito AS c2 JOIN dlcs AS d ON d.IdDlcs = c2.juegos_id WHERE c2.usuario_id=:id AND c2.estado = '0'");
  $sentenciaSQL->bindParam(':id', $_SESSION['idCliente']);
  $sentenciaSQL->execute();
  $carrito2 = $sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);

  $sentenciaSQL = $conexion->prepare("SELECT c3.*,co.NombreConsolas,co.PrecioConsolas,co.ImagenConsolas FROM carrito AS c3 JOIN consolas AS co ON co.IdConsolas = c3.juegos_id WHERE c3.usuario_id=:id AND c3.estado = '0'");
  $sentenciaSQL->bindParam(':id', $_SESSION['idCliente']);
  $sentenciaSQL->execute();
